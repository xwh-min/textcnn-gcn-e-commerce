...
const mockUsers: User[] = [
  ...
  { id: '2', username: 'user1', email: 'user1@example.com', role: '普通用户', status: 'active', createdAt: '2026-02-15' },
  { id: '3', username: 'user2', email: 'user2@example.com', role: '普通用户', status: 'disabled', createdAt: '2026-03-01' },
];

const mockRoles: Role[] = [
  { id: '1', name: '管理员', description: '系统管理员，拥有所有权限', permissions: ['dashboard_view', 'enterprises_view', 'enterprises_add', 'enterprises_edit', 'enterprises_delete', 'graph_view', 'datacenter_view', 'datacenter_import', 'datacenter_export', 'detection_single', 'detection_batch', 'risks_view', 'risks_export', 'system_users', 'system_roles', 'system_logs'] },
  { id: '2', name: '普通用户', description: '普通用户，基础权限', permissions: ['dashboard_view', 'enterprises_view', 'graph_view', 'datacenter_view', 'detection_single', 'risks_view'] },
];

const mockLogs: Log[] = [
  { id: '1', operator: 'admin', action: '登录系统', type: '登录', time: '2026-04-09 10:30:00', details: '用户 admin 登录系统' },
  { id: '2', operator: 'admin', action: '新增用户', type: '用户管理', time: '2026-04-09 10:25:00', details: '新增用户 user3' },
  { id: '3', operator: 'user1', action: '风险预测', type: '风险预测', time: '2026-04-09 09:15:00', details: '对企业A进行单次风险预测' },
];
...
export default function SystemPage() {
    ...
    try {
      ...
      if (usersRes.code === 200 && usersRes.data) {
        setUsers(usersRes.data.map((item: any) => ({
          ...
          email: item.email || '',
          role: item.role || item.roleName || '普通用户',
  ...
  const handleToggleUserStatus = (user: User) => {
    setUsers(users.map(u => u.id === user.id ? { ...u, status: u.status === 'active' ? 'disabled' : 'active' } : u));
  };

  const handleResetPassword = (user: User) => {
    alert(`已为用户 ${user.username} 重置密码为：123456`);
  };
  ...
  const handleSaveRole = () => {
    if (currentRole) {
      setRoles(roles.map(r => r.id === currentRole.id ? { ...r, permissions: selectedPermissions } : r));
    }
                  ...
                  <tbody>
                    {filteredLogs.map(log => (
                      <tr key={log.id}>
                        ...
                        <td>{log.time}</td>
                        <td>
                          <button className="detail-btn" onClick={() => alert(log.details)}>
                            查看详情
                          </button>
                        </td>
                      </tr>
