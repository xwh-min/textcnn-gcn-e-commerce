...
export default function ComplaintsPage() {
  ...
  const loadComplaints = async () => {
    try {
      setLoading(true);
      setError(null);
      const response = await apiService.getUserComplaints();
  ...
  const handleMarkResolved = async (record: any) => {
    try {
      const response = await apiService.markComplaintResolved(parseInt(record.id));
  ...
  const handleDelete = async (record: any) => {
    if (confirm('确定要删除这条投诉吗？')) {
      try {
        const response = await apiService.deleteUserComplaint(parseInt(record.id));
        if (response.code === 200) {
          await loadComplaints();
        } else {
