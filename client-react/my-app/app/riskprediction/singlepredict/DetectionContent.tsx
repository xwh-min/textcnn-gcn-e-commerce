...
export default function DetectionContent() {
  ...
  const fetchRiskPrediction = async () => {
    ...
    try {
      const data = await post<ApiResponse<RiskPredictionResult>>(
        API_CONFIG.ENDPOINTS.RISK_PREDICT,
        {
      ...
      if (data && data.result) {
        setResult(data.result as unknown as RiskPredictionResult);
      }
    } catch (err) {
