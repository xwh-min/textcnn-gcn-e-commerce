...
type RiskPredictionService struct {
	inference      *inference.ONNXInference
...
func (s *RiskPredictionService) Predict(ctx context.Context, req *RiskPredictionRequest) (*RiskPredictionResult, error) {
	...
	output, err := s.inference.Predict(combinedFeatures)
	if err != nil {
	...
	log.Printf("Risk prediction completed - Compliance: %s (%.2f), Payment: %s (%.2f)",
