...
type OrderData struct {
	...
	PaymentStatus      string    `gorm:"column:payment_status;size:50"`                                       // 支付状态（success/failed/pending）
	ExtraInfo          string    `gorm:"column:extra_info;type:jsonb"`                                        // 扩展字段（存订单详情、商品信息等半结构化数据）
