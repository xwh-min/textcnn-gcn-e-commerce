    ...
    def train_epoch(self, data_loader):
        ...
        for batch in data_loader:
            ...
            for i in range(len(text_input)):
                ...
                single_edge_index = edge_index[i] if hasattr(edge_index, '__getitem__') else edge_index
                single_compliance_label = compliance_labels[i].unsqueeze(0)
                single_payment_label = payment_labels[i].unsqueeze(0)
                
                # 处理文本输入
                if hasattr(self.model, 'text_extractor_type'):
                    if self.model.text_extractor_type in ['textcnn', 'both']:
                        # 对于TextCNN，需要将文本转换为词索引
                        # 这里简化处理，实际应该使用更复杂的文本处理
                        # 临时创建一个简单的词索引转换
                        words = single_text_input.split()
                        # 确保文本长度至少为最大卷积核大小
                        max_kernel_size = 5  # 对应textcnn_params中的最大卷积核大小
                        if len(words) < max_kernel_size:
                            # 填充文本，确保长度足够
                            words = words + ['<PAD>'] * (max_kernel_size - len(words))
                        word_to_idx = {word: i+1 for i, word in enumerate(set(words))}  # 0为padding
                        single_text_input = torch.tensor([word_to_idx.get(word, 0) for word in words], dtype=torch.long)
                        single_text_input = single_text_input.unsqueeze(0)  # 添加批次维度
